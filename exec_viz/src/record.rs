...
//! and zero_fee_arb each hand-roll today, standardized here. Schema-only for now — writers come
//! with the first migration. Row shapes are modeled on `scam_pump_liqs/viz/src/read.rs`
//! (`OrderEventRow`, `DrawingRow`), which is the proven read-side contract.
